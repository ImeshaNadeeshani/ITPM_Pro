...
const TimeTables = () => {
  ...
  useEffect(() => {
    axios.get(`/api/semesters/${userName}`)
      .then((res) => {
  ...
  const handleCreateTable = async (e) => {
    e.preventDefault();
    await axios.post("/api/createTable", { semester, institution, comments, user_name: userName });
    alert("Table Created!");
    setSemester("");
    setInstitution("");
    setComments("");
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleCreateTable}>
        ...
        <input type="text" placeholder="Institution Name" value={institution} onChange={(e) => setInstitution(e.target.value)} required />
